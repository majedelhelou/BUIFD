...
from models import DnCNN_RL, BUIFD, BUIFD_name2
from utils import *
...
def inference(test_data, model, varying_noise=False, color_mode='gray'):
    files_source = glob.glob(os.path.join('Data', test_data, '*.png'))
    ...
    std_psnr_results = np.zeros((len(files_source), len(std_values)))
        ...
        for noise_idx, noise_std in enumerate(std_values):
            ...
            with torch.no_grad():
                if  opt.model == 'R':
                    ...
                    std_psnr_results[img_idx, noise_idx] = std_psnr

        img_idx += 1
        
    return psnr_results, std_psnr_results
...
def main():
    ...
    if opt.model == 'R':
        ...
        if opt.max_train_noise == 55 and opt.color_mode == 'color':
            #this is the same network as BUIFD, only variable naming conventions are different (check the comments in Inference_Pretrained/models.py)
            #it is fixed in the training codes, but kept here to reuse the exact same pretrained models as those of the paper
            net = BUIFD_name2(channels=model_channels, num_of_layers=num_of_layers)
        else:
    ...
    psnr_results, std_psnr_results = inference(test_data, model, opt.varying_noise, opt.color_mode)


    if opt.model == 'F':
        np.save( os.path.join( log_dir, ('varN_' if opt.varying_noise else '')  + 'PSNR_' + str(opt.epoch)), psnr_results )
        np.save( os.path.join( log_dir, ('varN_' if opt.varying_noise else '')  + 'PSNR_std_' + str(opt.epoch)), std_psnr_results )
    else:
        np.save( os.path.join( log_dir, ('varN_' if opt.varying_noise else '') + 'PSNR_' + str(opt.epoch)), psnr_results )
